...
- **Kaub 水位换算**：GloFAS 模型格点在 Kaub 站精确坐标上落在流量极小的支汊，故改用附近能正确
  解析莱茵河主河道的格点（约5km外）。换算曲线最早用最近45天实测水位/流量拟合了一条幂函数——
  在校准窗口内很准（R²≈0.8），但外推到窗口外的流量（比如冬季洪峰）时会指数级发散，曾经算出
  过10000+cm的荒谬水位。已改用 Kaub 站官方长期特征值（Pegelonline 的 `characteristicValues` +
  BfG 发布的流量极值统计，NQ/MNQ/MQ/MHQ/HQ 对应 NNW/MNW/MW/MHW/HHW，覆盖1856年至今的实测
  范围）做对数空间分段插值，超出官方记录范围（25-825cm）直接截断、不再外推，纵轴永远落在物理
  合理区间内；页面显示的置信度改成了拿最近45天实测数据做样本外校验的 R²（更保守但更诚实）。
  这仍是分析师近似方法，不是 WSV 官方水位-流量关系表，仅用于判断趋势与阈值穿越时点，精确调度/
  航运决策请以 WSV/ELWIS 官方数据为准。
